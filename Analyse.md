...
Im Verlauf des Projekts werde ich mehrere Schritte durchführen. Zunächst werde ich den Datensatz auswählen und die Qualität überprüfen. Danach werde ich wichtige Variablen beschrieben und anhand statistischer Kennzahlen analysieren. Zunächst werde ich die
Verteilung und Streuung der Daten sowie die grafische Darstellung mithilfe geeigneter Diagramme untersuchen. Zum Abschliessen werde ich mögliche Zusammenhänge zwischen verschiedenen Einflussfaktoren und der schulischen Leistung analysieren und interpretieren.
...
Beispiele aus dem Datensatz:
- Gesamtnote (overall_score)
Die Punktzahlen (z. B. overall_score) besitzen formal einen natürlichen Nullpunkt und könnten daher als verhältnisskaliert betrachtet werden. Jedoch werden sie in der praktischen statistischen Analyse häufig wie intervallskalierte Daten behandelt, da
hauptsächlich die Unterschiede zwischen den Werten und nicht deren Verhältnisse im Vordergrund stehen.
...
Der Modus deutet darauf hin, dass besonders viele Schüler/innen die maximale Punktzahl erreicht haben.
Allerdings ist der Modus in diesem Fall nur bedingt aussagekräftig, da viele Werte nur selten oder gleich häufig auftreten. Daher eignet sich der Median in diesem Fall besser, um die zentrale Tendenz der Daten zu beschreiben.
Minimum und Maximum
Der Minimum und Maximum zeigen den kleinsten und grössten Wert einer Datenreihe.
Sie geben einen Überblick über den gesamten Wertebereich und legen die Grundlage fest für die Berechnung der Spannweite.
In diesem Fall ist:
...
Hier beträgt die Varianz: 358.556739 Punkte² -> gerundet: 358.6 Punkte²
...
Hier beträgt die Standardabweichung: 18.93559449819308 Punkte -> gerundet: 18.9 Punkte
...
Ich habe mich bewusst für Boxplot, Histogramm und Balkendiagramme entschieden, da sie jeweils verschiedene Aspekte der Daten darstellen. Besonders hilfreich war es, die
Diagramme nicht nur zu beschreiben, sondern auch zu erklären, warum ich sie verwendet habe.
...
Korrelationskoeffizient: 0.905771445
...
Korrelationskoeffizient: 0.292761519
...
Korrelationskoeffizient: 0.002691873
...
Quellen / Anhang
Beispiel-Portfolio:
- e-Portfolio von Andrin Hutterli: Portfolio
Datenquelle:
- Kaggle: Link
...
Abschnitt HZ1:
- Bild
- Arbeitsmappe: Student_Performance.xlsx
Abschnitt HZ2:
- Bild
- Arbeitsmappe: Student_Performance.xlsx
Abschnitt HZ3:
...
Abschnitt HZ4:
- Bild
- Diagramme aus Excel-Datei: Arbeitsmappe "Student_Performance_HZ4"
- Arbeitsmappe: Student_Performance.xlsx
Abschnitt HZ5:
- Bild
- Arbeitsmappe: Student_Performance.xlsx
...
- "Wie kann ich in Excel bspw. study_hours in Bereiche Gruppieren? Bspw. 1-2 Stunden, 3-4 Stunden" (Code kopiert)
- "Wie kann man den Wert, um als Ausreisser zu gelten, berechnen?"
- "Was ist der Unterschied zwischen =QUARTILE.INKL und =QUARTILE.EXKL und was würdest du in meinem Fall empfehlen?"
Kopiertes Code:
- =WENN(A2<=2;"1-2 Stunden";WENN(A2<=4;"3-4 Stunden";WENN(A2<=6;"5-6 Stunden";"mehr als 6 Stunden")))
